...
const TokenRow = ({ token, onSelect }: { token: any, onSelect: any }) => (
  <tr className="border-b border-gray-700 hover:bg-gray-800 cursor-pointer" onClick={() => onSelect(token)}>
    <td className="py-4 flex items-center">
      <img src={token.logo} alt={token.name} className="w-8 h-8 mr-3 rounded-full" />
      <div>
...
export const WalletInteractions = () => {
  const { deployToken, deployNFtToken } = useAccount();
  ...
  const [deployNFTTokenAddress, setDeployNFTTokenAddress] = useState('');

  useEffect(() => {
    // Simulated token data
    setTokens([
      { id: 1, name: 'USD Coin', symbol: 'USDC', logo: '/usdc-logo.svg', portfolio: 81.09, price: 1.00, priceChange: -0.14, balance: 279.73, amount: 280.0713 },
      { id: 2, name: 'Dai Stablecoin', symbol: 'DAI', logo: '/dai-logo.svg', portfolio: 15.44, price: 1.00, priceChange: -0.11, balance: 53.26, amount: 53.3194 },
      { id: 3, name: 'Ether', symbol: 'ETH', logo: '/eth-logo.svg', portfolio: 3.48, price: 2323.93, priceChange: -1.03, balance: 11.99, amount: 0.0052 },
    ]);
  }, []);
  ...
  const handleDeplyNftToken = async () => {
    ...
    try {
      const nftContract = await deployNFtToken(currentWallet);
      console.log('NFT Token deployed at:', nftContract.address.toString());
  ...
  const handleMint = async (isPublic: boolean) => {
    ...
    setIsLoading({ ...isLoading, [isPublic ? 'mintPublic' : 'mintPrivate']: true });
    try {
      let tx;
      if (isPublic) {
        tx = await tokenContract.methods.mint_public(currentWallet.getAddress(), 100n).send();
      } else {
      ...
      setIsLoading({ ...isLoading, [isPublic ? 'mintPublic' : 'mintPrivate']: false });
    }
  };
  const checkBalance = async (isPublic = true) => {
    ...
    try {
      ...
      const balance = await tokenContract.methods[method](currentWallet.getAddress()).simulate();

      toast.success(
        `${isPublic ? 'Public' : 'Private'} Balance of address ${currentWallet.getAddress().toShortString()}: ${balance}`
      );
    } catch (error) {
      console.error('Error checking balance:', error);
      toast.error(`Error checking ${isPublic ? 'public' : 'private'} balance: ${(error as Error).message}`);
    }
  };

  const handleTransfer = async (isPublic: boolean) => {
    if (!tokenContract || !currentWallet || !recipientAddress || transferAmount <= 0) {
      toast.error('Please fill in all transfer details');
      return;
    }
    setIsLoading({ ...isLoading, [isPublic ? 'transferPublic' : 'transferPrivate']: true });
    try {
      let tx;
      if (isPublic) {
        tx = await tokenContract.methods
        ...
        tx = (await TokenContract.at(tokenContract.address, currentWallet)).methods
      ...
      setIsLoading({ ...isLoading, [isPublic ? 'transferPublic' : 'transferPrivate']: false });
    }
  };

  const handleMovingTokens = async (isPublic = true) => {
    if (!recipientAddress || transferAmount === 0 || !tokenContract || !currentWallet) {
      return toast.error('Invalid call: Please check recipient address, transfer amount, and contract/wallet availability.');
    }
  
    try {
      setIsLoading({ ...isLoading, [isPublic ? 'movingPublic' : 'movingPrivate']: true });
      ...
      <main className="p-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-black bg-opacity-50 rounded-lg p-6 backdrop-blur-sm">
            ...
            {/* <WalletSection wallets={wallets} setWallets={setWallets} setCurrentWallet={setCurrentWallet} /> */}
            <div className='mb-4 mt-4 w-full'><button
            ...
            >
              {isLoading.deployToken ? <Loader className="animate-spin mr-2" /> : null}
              Deploy Token
            </button></div>
            ...
            >
              {isLoading.deployNFTToken ? <Loader className="animate-spin mr-2" /> : null}
              Deploy NFT Contract
            </button></div>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <button
                ...
                className="bg-gradient-to-r from-blue-500 to-purple-500 px-4 py-2 rounded-full text-sm hover:from-blue-600 hover:to-purple-600 transition-all"                disabled={isLoading.mintPublic}
              >
                {isLoading.mintPublic ? <Loader className="animate-spin mr-2" /> : null}
                Mint Public
              </button>
              <button
                onClick={() => handleMint(false)}
                className="bg-gradient-to-r from-blue-500 to-purple-500 px-4 py-2 rounded-full text-sm hover:from-blue-600 hover:to-purple-600 transition-all"                disabled={isLoading.mintPrivate}
              >
                {isLoading.mintPrivate ? <Loader className="animate-spin mr-2" /> : null}
                Mint Private
              </button>
              <button
                onClick={() => checkBalance(true)}
                className="bg-gradient-to-r from-blue-500 to-purple-500 px-4 py-2 rounded-full text-sm hover:from-blue-600 hover:to-purple-600 transition-all"                disabled={isLoading.checkBalancePublic}
              >
                Public Balance
              </button>
              <button
                onClick={() => checkBalance(false)}
                className="bg-gradient-to-r from-blue-500 to-purple-500 px-4 py-2 rounded-full text-sm hover:from-blue-600 hover:to-purple-600 transition-all"                disabled={isLoading.checkBalancePrivate}
              >
                Private Balance
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                >
                  {isLoading.transferPublic ? <Loader className="animate-spin mr-2" /> : null}
                  Public Transfer
                </button>
                <button
                ...
                >
                  {isLoading.transferPrivate ? <Loader className="animate-spin mr-2" /> : null}
                  Private Transfer
                </button>
                <button
                ...
                >
                  Move To Public
                  {isLoading.movingPublic ? <Loader className="animate-spin mr-2" /> : null}
                </button>
                <button
                ...
                >
                  Move To Private
                  {isLoading.movingPrivate ? <Loader className="animate-spin mr-2" /> : null}
                </button>
              </div>
          ...
          {tab === 'Transactions' && <div className="bg-black bg-opacity-50 rounded-lg p-6 backdrop-blur-sm overflow-auto">
            ...
            <div className="items-center space-x-4 ">
              <span className="text-2xl font-bold">Deployed NFT Token Address:</span>
              <span className="text-gray-400">{deployNFTTokenAddress}</span>
            </div>
           </div>}
          ...
          <div className="mt-6 bg-black bg-opacity-50 rounded-lg p-6 backdrop-blur-sm">
            ...
            <table className="w-full">
              ...
              <tbody>
                {tokens.map((token) => (
                  <TokenRow key={token.id} token={token} onSelect={setSelectedToken} />
                ))}
              </tbody>
            </table>
          </div>

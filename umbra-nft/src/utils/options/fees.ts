...
import { parseBigint, aliasedAddressParser } from './cli-utils.js'
...
export function printGasEstimates(
  ...
  log(
    `Maximum total tx fee:   ${getEstimatedCost(
      gasEstimates,
      inclusionFee,
...
export class FeeOpts implements IFeeOpts {
  ...
  // static getOptions() {
  //   return [
  //     new Option('--inclusion-fee <value>', 'Inclusion fee to pay for the tx.').argParser(
  //       parseBigint
  //     ),
  //     new Option(
  //       '--gas-limits <da=100,l2=100,teardownDA=10,teardownL2=10>',
  //       'Gas limits for the tx.'
  //     ),
  ...
  static fromCli(args: CliFeeArgs) {
    ...
    return new FeeOpts(
      estimateOnly,
      gasSettings,
      () => Promise.resolve(new NoFeePaymentMethod()),
      !!args.estimateGas
    )
  }
}
    ...
    switch (parsed.method) {
      ...
      case 'native':
        if (parsed.claim || (parsed.claimSecret && parsed.claimAmount)) {
          ...
          if (parsed.claim && db) {
            ;({ amount: claimAmount, secret: claimSecret } = await db.popBridgedFeeJuice(
              sender.getAddress(),
              log
            ))
          } else {

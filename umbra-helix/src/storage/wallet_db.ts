...
export class WalletDB {
  ...
  async pushBridgedFeeJuice(recipient: AztecAddress, secret: Fr, amount: bigint, log: LogFn) {
    let stackPointer =
      this.#bridgedFeeJuice.get(`${recipient.toString()}:stackPointer`)?.readInt8() || 0
    stackPointer++
  ...
  async storeAuthwitness(authWit: AuthWitness, log: LogFn, alias?: string) {
    ...
    log(
      `Authorization witness stored in database with alias${alias ? `es last & ${alias}` : ' last'}`
    )
  }
  ...
  retrieveTxData(txHash: TxHash) {
    ...
    const cancellable =
      this.#transactions.get(`${txHash.toString()}:cancellable`)!.toString() === 'true'

...
export async function addScopeToWallet(
  ...
  if (db) {
    await db.storeAccountMetadata(
      wallet.getAddress(),
      'scopes',
      Buffer.from(deduplicatedScopes.join(','))
    )
  }
...
export async function getWalletWithScopes(account: AccountManager, db?: WalletDB) {
  ...
  if (db) {
    ...
    try {
      storedScopes = (db.retrieveAccountMetadata(wallet.getAddress(), 'scopes') ?? '')
        .toString()
        .split(',')
    ...
    const deduplicatedScopes = Array.from(
      new Set([address, ...currentScopes, ...storedScopes]).values()
    ).map((scope) => AztecAddress.fromString(scope))
